...
cd $pythonDir
if [[ $nuitka == true ]]; then
    export PYTHONPATH="./grpc_service"
    $PYTHON -m nuitka server.py --standalone --onefile --output-dir=./dist --output-filename="$exeNameFull"
else
    $PYTHON -m PyInstaller --onefile --noconfirm --clean --log-level=WARN --name="$exeNameFull" --paths="./grpc_service" server.py
fi
cd $workingDir
...
numLines=$(wc -l < "$flutterDir"/lib/utils/service/py_file_info.dart)
lastLine=$((numLines - 2))
head -n $lastLine "$flutterDir"/lib/utils/service/py_file_info.dart > "$flutterDir"/lib/utils/service/py_file_info_temp.dart
mv "$flutterDir"/lib/utils/service/py_file_info_temp.dart "$flutterDir"/lib/utils/service/py_file_info.dart

echo "const exeFileName = '$exeName';" >> "$flutterDir"/lib/utils/service/py_file_info.dart
echo "const currentFileVersionFromAssets = '$currentDateTime';" >> "$flutterDir"/lib/utils/service/py_file_info.dart
